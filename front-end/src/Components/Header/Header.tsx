...
const Header = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const result = (
                <div className="bg-zinc-300 h-64 overflow-y-scroll w-9/12 rounded">
                    {filter.map((item) => {
                        return (
                            <Fragment key={item.id}>
                                <div
                                    ...
                                    className="w-full w-min-full bg-red flex m-auto hover:bg-zinc-400 p-2"
                                >
                                    ...
                                    <div className="w-9/12 ml-2">
                                        <p>{item.name}</p>
                                        <p className="font-black font-mono text-black">{item.price} €</p>
                                    </div>
                                </div>
                            </Fragment>
                        );
    ...
    return (
        <>
            <header className="bg-slate-800 mx-3 md:mx-24 mb-3 px-2 md:px-5 py-4 rounded">
                    ...
                    <div className="w-full md:w-5/12 lg:w-6/12 mb-3 md:mb-0 md:ml-auto md:mr-auto">
                        <div className="flex relative">
                            <select
                                ...
                                className="w-3/12 mr-4 bg-white px-2 py-1 rounded"
                            >
                                <option
                                    defaultValue={""}
                                    value={JSON.stringify({})}
                                >Filtres</option>
                                {categoriesSearch.map((item: any) => {
                                    item.subcate = false;
                                    return (
                                        <Fragment key={item.id}>
                                            <option
